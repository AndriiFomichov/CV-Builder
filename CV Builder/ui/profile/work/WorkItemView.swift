    ...
    var body: some View {
        Button (action: clickHandler) {
            HStack (spacing: 0) {
                ...
                VStack {
                    Text(work.company).font(.title2).bold().foregroundStyle(.accent).frame(maxWidth: .infinity, alignment: .leading).multilineTextAlignment(.leading).lineLimit(1)
                    
                    Text(work.jobTitle.isEmpty ? work.location :work.jobTitle).font(.subheadline).foregroundStyle(.textAdditional).frame(maxWidth: .infinity, alignment: .leading).multilineTextAlignment(.leading).lineLimit(1)
