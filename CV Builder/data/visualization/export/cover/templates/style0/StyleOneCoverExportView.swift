...
struct StyleOneCoverExportView: View {
    ...
    var body: some View {
        HStack (spacing: 0) {
            ...
            }.frame(maxHeight: .infinity).padding(CGFloat(cv.marginsSize)).padding(.vertical, CGFloat(cv.marginsSize)).overlay {
...
#Preview {
    StyleOneCoverExportView(cv: CVEntityWrapper.getDefault(), addWatermark: true)
}
