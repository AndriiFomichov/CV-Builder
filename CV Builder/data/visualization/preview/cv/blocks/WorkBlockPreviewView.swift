    ...
    var body: some View {
        HStack (alignment: .top, spacing: 0) {
            ...
            if block.isMainBlock && block.styleDateSeparated && !block.styleDateWithHeader && (item.startDate != nil || item.endDate != nil) {
                VStack {
                    TextPreviewView(text: getDateText(), font: cv.textFont, color: mainTextColor, gravity: .leading, size: cv.textSize, isBold: false, isItalic: false, isUnderline: false, isUppercased: false)
                ...
                HStack (alignment: .top, spacing: CGFloat(cv.marginsSize / 3)) {
                    ...
                    VStack (spacing: CGFloat(cv.marginsSize / 4)) {
                        if (!block.isMainBlock || !block.styleDateSeparated) && (item.startDate != nil || item.endDate != nil) && !block.styleDateWithHeader {
                            TextPreviewView(text: getDateText(), font: cv.textFont, color: mainTextColor, gravity: .leading, size: cv.textSize, isBold: false, isItalic: false, isUnderline: false, isUppercased: false).opacity(0.5)
                        }
